        ...
        int modoVideo, char *tituloJanela)
{
    ...
    // Imprime informacao do modo de vídeo requisitado
    printf("Iniciando o modo de video (OpenGL): %ix%ix32bpp\n", largura, altura);
    
    // Flags do modo de video (se ficar assim, roda em janela)
    Uint32 flags = SDL_WINDOW_OPENGL; // | SDL_GL_DOUBLEBUFFER;
	...
	int atributo;
        ...
        if(atributo != 0)
                        printf("A aplicacao esta utilizando double buffer como esperado.\n");
        else
                        printf("ATENCAO: a aplicacao nao esta usando double buffer. Para melhores resultados, utilize um hardware que o suporte.\n");
...
/*
    if(SDL_VideoModeOK(largura, altura, 32, flags) )
    {
        ...
        SDL_GL_SetAttribute( SDL_GL_DOUBLEBUFFER, 1 );  
	SDL_GL_SetSwapInterval(1);	
	// REMOVER SDL_GL_SetAttribute( SDL_GL_SWAP_CONTROL, 1 );
    ...
    {
        printf("O modo de video nao eh suportado pelo hardware. Encerrando.");; 
		SDL_QuitSubSystem(SDL_INIT_VIDEO);
        return false;
    }
    printf("Modo de video iniciado com sucesso!\n");
...
*/
    ...
    // REMOVER SDL_GL_SwapBuffers();
    // Testa o tamanho da textura
	glGetIntegerv(GL_MAX_TEXTURE_SIZE, &maxtexsize);
	
    printf("O tamanho maximo de textura suportado eh de %ix%i pixels.\n", maxtexsize, maxtexsize);
...
unsigned int C2D2GL_CarregaSpriteSet(const char *arquivo, int largura, int altura)
{
	...
	if(largura!=0 && altura!=0)
	{
		...
		{
			printf("A imagem %s nao pode ser usada como fonte de sprites de tamanho %ix%i\n.",
	...
	if(sprites[idx].imagem->w != sprites[idx].imagem->h || !potencia || sprites[idx].imagem->format->BytesPerPixel!=4)
	{
        ...
        if(sprites[idx].imagem == 0) 
        {
			printf("Nao conseguiu criar a imagem temporaria ao carregar o sprite set. Arquivo: %s. Erro: %s.\n", 
				arquivo, SDL_GetError());
	        SDL_FreeSurface(sprites[idx].imagem);
			sprites[idx].imagem = 0;
			sprites[idx].apelido[0] = '\0';
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, sprites[idx].imagem->w,
                sprites[idx].imagem->h, 0, GL_BGRA, GL_UNSIGNED_BYTE,
