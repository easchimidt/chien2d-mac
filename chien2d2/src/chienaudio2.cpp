...
bool CA2_Inicia()
{
    ...
    {        
		printf("Nao conseguiu iniciar a Chien Audio 2 (subsistema SDL). Erro do sistema: %s\n", SDL_GetError());
        return false;
    }
	...
	if(Mix_OpenAudio(44100, AUDIO_S16SYS, 2, CA2_BUFFER_MIXER) < 0)
    {
    	...
    	printf("Nao conseguiu iniciar a Chien Audio 2 (formato de audio). Erro do sistema: %s\n", SDL_GetError());
    	return false;
   	}
...
bool CA2_AjustaVolume(unsigned int musica, unsigned int efeitos)
{
	...
	if(!CA2_inicializado)
	{
		printf("Atencao: tentativa de mudar o volume do audio sem iniciar o sistema de audio!\n");
		return false;
	}
...
unsigned int CA2_CarregaMusica(const char *arquivo)
{
    ...
    if(idx==0)
	{
		printf("Estourou o limite de musicas carregadas no sistema. \n");
        // Retorna um índice inválido para indicar que o método falhou
        return 0;
	}
